  ...
  channels()
{
  ...
  music = Mix_LoadMUS("sound/loop.wav");
  if (!music) throw string("Couldn't load 100Years.mp3")+Mix_GetError();
  startMusic();

  sounds.push_back( Mix_LoadWAV("sound/death.M.wav") );
  sounds.push_back( Mix_LoadWAV("sound/swing.M.wav") );
  sounds.push_back( Mix_LoadWAV("sound/Hit.wav") );
  sounds.push_back( Mix_LoadWAV("sound/spider_attack.M.wav") );
  for (unsigned int i = 0; i < sounds.size(); ++i) channels.push_back(i);
  std::cout << "Music and Sound is loaded" <<sounds.size()<< std::endl;
}
